...
function AddBlock() {
    var newElement = '<div class="row form-group block">' +
        '<div class="card card-body border-dark">' +
        '<div class="row form-group text-dark">' +
        '<div class="col-md-5">' +
        '<input type="text" name="Blocks[' + counter + '].Header" class="form-control headerInput" placeholder="Заголовок поля">' +
        '</div>' +
        '<div class="col-md-5">' +
        '<select class="custom-select" name="Blocks[' + counter + '].FieldType">' +
        ...
        '<textarea name="Blocks[' + counter + '].TextField" class="form-control blockArea"  rows="2"></textarea>' +
        '</div>' +
        '</div>' +
        '<div class="row check-style justify-content-end">' +
        '<div class="custom-control custom-checkbox ">' +
        '<label>' +
        '<input type="checkbox" class="checkboxMand" name="Blocks[' + counter + '].Mandatory" >' +
    ...
    $('.formContainer').append(newElement);
    $(".delete").click(RemoveBlock);
    $(".custom-select").click(SetSelectChanged);
    //RecalculateIndexes();
}
